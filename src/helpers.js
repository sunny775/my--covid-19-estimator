...
function estimateSevereCases(data, impact) {
  var projectedInfections = impact
    ? estimateProjectedInfections(data, impact)
...
function estimateBedSpaceAvialability(data, impact) {
  var severeCases = impact
    ? estimateSevereCases(data, impact)
...
function estimateDollarsInFlight(data, impact) {
  ...
  return (
    projectedInfections * avgDailyIncomeInUSD * avgDailyIncomePopulation * days
  );
}
